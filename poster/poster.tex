...
\authors{2조 - 20212978 김신영, 20212952 강민성}
...
\begin{bpbox}[bpcolor=grey, bpstyle=wide, title={배경}]
    ...
    IDE, 컴파일러, CI 등 다양한 개발 과정에서 코드 품질과 안정성을 높이기 위해 활용되고 있음.
...
% --- BLOCK 3: Methods ---
\begin{bpcolumns}[2]
    \begin{bpbox}[title={분석 예시: 무한 루프 및 Dead Code Detection}, bptitlebg=transparent]
        \noindent
...
func main() {
    ...
    Print(a)     a: $\bot$
        ...
        \begin{minipage}{0.45\textwidth}
            ...
            무한히 반복되는 코드의 경우 계산된 상태에 대해 widening 기법을 적용하여 분석이 종료할 수 있도록 유도.
            이후 filtering을 통해 결과를 정제하여 정확도 손실을 방지. 
...
\begin{bpcolumns}[2]
    \begin{bpbox}[bpcolor=grey, bpstyle=wide, title={시각화 및 인터페이스}]
        \begin{minipage}{0.48\textwidth}
            \begin{center}
                    \includegraphics[width=\linewidth, height=16.5cm, keepaspectratio]{images/Inspector2.png}%
            \end{center}
        \end{minipage}
        \begin{minipage}{0.48\textwidth}
            \begin{center}
                    \includegraphics[width=\linewidth, height=16.5cm, keepaspectratio]{images/Inspector3.png}%
            \end{center}
        \end{minipage}

        \kern25pt
        \begin{compactitem}
            \item 분석 과정에서 생성되는 상태 변화를 단계별로 재생 가능하도록 구현.
            \item 함수별 및 단계별 상태를 자유롭게 탐색 가능.
        \end{compactitem}
\end{bpbox}
\begin{bpbox}[bpcolor=grey, bpstyle=wide, title={한계 및 향후 계획}]
    \begin{itemize}
        \item 관계형 요약도메인 : \texttt{i < len(arr)} 등과 같이 구체적인 값이 아닌 변수간의 논리적 관계를 나타낼 수 있는 대수적 도메인.
        \item 분석 정확도 개선 : reduced product domain, narrowing 구현 및 SimpleProp 체계 확장.
        \item Pointer inference : 포인터 및 메모리 동작 추상화를 통한 포인터 연산 검증.
        \item \inleq{\mathsf{C} \rightarrow \mathsf{Imp}, \mathsf{Rust} \rightarrow \mathsf{Imp}} 등 \inleq{\mathsf{Imp}}를 활용한 다양한 언어 지원.
        \item 보다 다양한 언어 기능 지원 : 확장된 타입 체계 등.
    \end{itemize}
\end{bpbox}
\end{bpcolumns}
