...
\input{style/mid_section_title.tex}
\input{style/mid_presentation.tex}
...
이러한 관점에서 정적 프로그램 분석은 의미론적 성질을 판별하는 문제이기에 결정 불가능(undecidable)하며, 이에 따라 "모든 프로그램"에 대해 "자동으로", "완벽한" 정확성을 가지는 정적 분석기는 구현이 불가능하다.
...
\begin{figure}[H]
    ...
    \end{lstlisting}
    \caption{인덱싱 오류가 존재하는 삽입 정렬 구현}
    \label{fig:wrong_insertionsort}
\end{figure}

코드 \ref{fig:wrong_insertionsort}는 배열 인덱싱 오류가 발생하는 삽입 정렬 구현체이다. 
\verb|j = i - 1| 대신 \verb|j = i| 를 사용했기에 \verb|arr[j], arr[j + 1]|이 위험한 코드이다.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/array_indexing_result.png}
    \caption{인덱싱 오류 분석 결과}
    \label{fig:array_indexing_result}
\end{figure}

그림 \ref{fig:array_indexing_result}는 분석기가 위 코드가 실행되는 시점에서 프로그램 상태를 나타낸다.
\traceinspector 분석기가 \verb|arr[j + 1] = key| 코드가 실행될 상태에서 배열의 길이는 $\interval{10}{10}$, \verb|j|의 값은
$\interval{-1}{9}$로 분석하였다. 따라서 $j + 1 = 10$인 경우가 가능하고, 이에 따라 \verb|arr[10]|이 가능한 상황이 발생하게 된다.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/oscillating_result.png}
    \caption{진동하며 발산하는 수열을 출력하는 코드의 분석 결과}
    \label{fig:oscillating_result}
\end{figure}

그림 \ref{fig:oscillating_result}는 $1, 2, -3, 4, -5, 6, -7, ...$와 같이 진동하며 발산하는 수열의 값을 무한히 출력하는 프로그램에 대해 분석을 수행한 결과를 나타낸다.
단순히 코드의 흐름에 따라 실행할 경우 분석이 영원히 끝나지 않지만, \traceinspector 분석기는 widening($\triangledown$)과 필터링 연산을 적용하여 루프 내부에서는 수열의 값이 $\interval{-\infty}{\infty}$가 가능함을 추론한다. 
또한 무한루프이기에 루프를 탈출한 이후에 실행되는 \verb|print(a)|가 도달 불가능하고, 이때 가능한 $a$의 값은 존재하지 않음($\bot$)을 추론한다. 
분석 결과가 고정점에 도달하기까지는 총 43단계가 소요되었다. 
또한 구간을 통해서만 구체적인 값을 요약하기에 1을 제외한 홀수의 경우는 임수, 모든 짝수는 양수 등의 보다 정확한 성질은 추론하지 못함을 확인할 수 있다.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/filter_result.png}
    \caption{루프 내에서와 탈출했을때의 변수값을 추론하는 결과}
    \label{fig:filter_result}
\end{figure}

그림 \ref{fig:filter_result}는 $a$가 $6$보다 작을 때까지 증가를 하다가 $6$이 되었을 때 루프를 탈출하는 코드와 분석 결과이다. 
코드는 무한 루프를 활용하기에 루프로부터 탈출하는 조건이 직접적으로 노출되지 않고, if와 break문의 조합을 통해 추론해야 한다. 
총 18단계에 걸쳐 계산된 고정점 상태에서 분석기는 루프 내부에서 가능한 $a$의 값이 $\interval{1}{5}$, 탈출한 이후에는 $\interval{6}{6}$임을 올바르게 추론한다.

이처럼 제어 흐름 그래프에 대한 고정점을 계산하고, widening과 필터링 연산을 통해 무한 루프에 대해서도 어느정도의 정확도를 가진 분석을 수행할 수 있다.
...
\begin{enumerate}
    \item 복잡한 구조보다는 프로젝트가 추구하는 바를 실현하기 위해, KISS(Keep It Simple and Stupid) 철학을 준수한다.
    \item 프론트엔드가 분석기인 만큼, 절대적인 안정성과 코드의 배포성을 지키기 위해 불필요한 라이브러리 사용은 지양한다.
    \item 기능은 물론이고, 사소한 디자인 또한 모든 브라우저에서 동일하게 적용함을 원칙으로 한다.
\end{enumerate}
...
처리가 완료가 되면, \verb|front_src/go/source.go|를 이용하여 분석기를 실행하고, 이 과정은 NodeJS 라이브러리 \verb|child_process| 내에 있는 \verb|exec| 함수를 이용한다.
...
\input{style/final_section_presentation_title.tex}
\input{style/final_presentation.tex}

\input{style/mentor_section_title.tex}
...
\input{style/meeting_section_title.tex}
