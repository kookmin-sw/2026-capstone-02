...
func (interpreter *AbstractAnalyzer[IntDomainImpl, ArrayDomainImpl]) Eval_expr(abs_state AbstractState[IntDomainImpl, ArrayDomainImpl], expr imp.Expr) AbstractValue[IntDomainImpl, ArrayDomainImpl] {
	...
	case *imp.NegExpr:
		...
		if subexpr_val.domain_kind != IntDomainKind {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("Result of arithmetic negation returned %s instead of IntDomain", subexpr_val.domain_kind))
		}
	...
	case *imp.AddExpr:
		...
		if !(lhs_val.domain_kind == IntDomainKind && rhs_val.domain_kind == IntDomainKind) {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, "Add expected LHS and RHS to be integer domain values, but are not")
		}
	...
	case *imp.SubExpr:
		...
		rhs_val := interpreter.Eval_expr(abs_state, expr_ty.Rhs)
		if !(lhs_val.domain_kind == IntDomainKind && rhs_val.domain_kind == IntDomainKind) {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, "Sub expected LHS and RHS to be integer domain values, but are not")
		}
	...
	case *imp.MulExpr:
		...
		rhs_val := interpreter.Eval_expr(abs_state, expr_ty.Rhs)
		if !(lhs_val.domain_kind == IntDomainKind && rhs_val.domain_kind == IntDomainKind) {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, "Mul expected LHS and RHS to be integer domain values, but are not")
		}
	...
	case *imp.DivExpr:
		...
		rhs_val := interpreter.Eval_expr(abs_state, expr_ty.Rhs)
		if !(lhs_val.domain_kind == IntDomainKind && rhs_val.domain_kind == IntDomainKind) {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, "Div expected LHS and RHS to be integer domain values, but are not")
		}
	...
	case *imp.ArrayIndexExpr:
		...
		if arr_val.domain_kind != ArrayDomainKind {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("'%s' : expected arr to have arr domain type", expr_ty))
		}
		if !index_val.Get_int().Leq(arr_val.Get_array().Len().Sub(index_val.int_domain.From_IntLitExpr(imp.IntLitExpr{Value: 1}))).IsTrue() {
			interpreter.output_handler.write_warning(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("Potentially unsafe array indexing: index has value %s, but %s.Len has value %s.", index_val.Get_int(), get_varname_from_lvalue(expr_ty.Base), arr_val.Get_array().Len()))
		}
	...
	case *imp.MakeArrayExpr:
		...
		if size_val.domain_kind != IntDomainKind {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("Size argument '%s' in make_array expected to have int domain type, but has type %s", expr_ty.Size, size_val.domain_kind))
		}
	...
	case *imp.EqExpr:
		...
		if lhs_val.domain_kind != rhs_val.domain_kind {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("'%s' : types of LHS and RHS are different", expr_ty))
		}
	...
	case *imp.LeqExpr:
		...
		if !(lhs_val.domain_kind == IntDomainKind && lhs_val.domain_kind == rhs_val.domain_kind) {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("'%s' : types of LHS and RHS are different (%s vs %s)", expr_ty, lhs_val.domain_kind, rhs_val.domain_kind))
		}
	...
	case *imp.CallExpr:
		...
		if !(func_exists) {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("Function %s does not exist", expr_ty.Func_name))
			panic("unreachable")
		}
		if len(func_info.Arg_pairs) != len(expr_ty.Args) {
			interpreter.output_handler.write_error(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("Function %s expectes %d arguments, but got %d", expr_ty.Func_name, len(func_info.Arg_pairs), len(expr_ty.Args)))
			panic("unreachable")
		}
		...
		if abs_state.remaining_call_depth == 0 {
			interpreter.output_handler.write_warning(abs_state.node_location, expr_ty.Line_num, fmt.Sprintf("Maximum function call depth(%d) reached. The call to %s will not be interpreted, but assumed to be ⊤.", interpreter.Settings.Max_call_stack_depth, expr_ty.Func_name))
...
func (interpreter *AbstractAnalyzer[IntDomainImpl, ArrayDomainImpl]) get_abstract_value_from_lvalue_expr(expr imp.Expr, state AbstractState[IntDomainImpl, ArrayDomainImpl]) AbstractValue[IntDomainImpl, ArrayDomainImpl] {
	...
	case *imp.VarExpr:
		...
		if !var_in_mem {
			interpreter.output_handler.write_error(state.node_location, expr_ty.Line_num, fmt.Sprintf("Variable '%s' not defined", expr_ty.Name))
		}
		return val
	case *imp.LenExpr:
		arr_val := interpreter.get_abstract_value_from_lvalue_expr(expr_ty.Subexpr, state)
		if arr_val.domain_kind != ArrayDomainKind {
			interpreter.output_handler.write_error(state.node_location, expr_ty.Line_num, fmt.Sprintf("Called len() on a non-array value '%s'", expr_ty.Subexpr))
		}
		return AbstractValue[IntDomainImpl, ArrayDomainImpl]{domain_kind: IntDomainKind, int_domain: arr_val.Get_array().Len()}
	case *imp.ArrayIndexExpr:
		arr_val := interpreter.get_abstract_value_from_lvalue_expr(expr_ty.Base, state)
		if arr_val.domain_kind != ArrayDomainKind {
			interpreter.output_handler.write_error(state.node_location, expr_ty.Line_num, fmt.Sprintf("Tried to index a non-array value '%s'", expr_ty.Base))
		}
		index_val := interpreter.Eval_expr(state, expr_ty.Index)
		if index_val.domain_kind != ArrayDomainKind {
			interpreter.output_handler.write_error(state.node_location, expr_ty.Line_num, fmt.Sprintf("Index expression '%s' is not an integerdomain", expr_ty.Index))
		}
		return arr_val.Get_array().GetIndex(index_val.Get_int())
	}
	interpreter.output_handler.write_error(state.node_location, expr.GetLineNum(), fmt.Sprintf("get_abstract_value_from_expr: Unsupported expression '%s'", expr))
	panic("unreachable")
}
...
func (interpreter *AbstractAnalyzer[IntDomainImpl, ArrayDomainImpl]) set_abstract_value_from_expr(lhs imp.Expr, rhs_val AbstractValue[IntDomainImpl, ArrayDomainImpl], state *AbstractState[IntDomainImpl, ArrayDomainImpl]) {
	...
	case *imp.VarExpr:
		...
		if lhs_exists && lhs_val.domain_kind != rhs_val.domain_kind {
			interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), fmt.Sprintf("LHS of variable %s has type %s, but RHS has type %s.", lhs_node.Name, lhs_val.domain_kind, rhs_val.domain_kind))
		}
	...
	case *imp.ArrayIndexExpr:
		...
		if index_val.domain_kind != IntDomainKind {
			interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), "Only 1-dimensional arrays implemented")
		}
		...
		if !lhs_exists {
			interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), fmt.Sprintf("Attempting to index nonexisting variable '%s'", arr_varname))
		}
		if lhs_val.domain_kind != ArrayDomainKind {
			interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), fmt.Sprintf("Attempting to index non-array variable '%s'", arr_varname))
		}
		...
		if !index_val.Get_int().Leq(lhs_val.Get_array().Len().Sub(index_val.int_domain.From_IntLitExpr(imp.IntLitExpr{Value: 1}))).IsTrue() {
			interpreter.output_handler.write_warning(state.node_location, lhs.GetLineNum(), fmt.Sprintf("Potentially unsafe array indexing: index has value %s, but %s.Len has value %s.", index_val.Get_int(), arr_varname, lhs_val.Get_array().Len()))
		}
	...
	case *imp.LenExpr:
		...
		lhs_val, lhs_exists := state.abstract_mem[arr_varname]
		if !lhs_exists {
			interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), fmt.Sprintf("Attempting to get length of nonexisting variable '%s'", arr_varname))
		}
		if lhs_val.domain_kind != ArrayDomainKind {
			interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), fmt.Sprintf("Attempting to get length of non-array variable '%s'", arr_varname))
		}
		if rhs_val.domain_kind != IntDomainKind {
			interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), fmt.Sprintf("subexpr of len is not an IntDomainKind, but %s", rhs_val.domain_kind))
		}
	...
	default:
		interpreter.output_handler.write_error(state.node_location, lhs.GetLineNum(), fmt.Sprintf("set_abstract_value_from_expr: Unsupported LHS expr '%s' with type %T", lhs, lhs))
	}
}
...
func (interpreter *AbstractAnalyzer[IntDomainImpl, ArrayDomainImpl]) Step(in_state AbstractState[IntDomainImpl, ArrayDomainImpl]) []AbstractState[IntDomainImpl, ArrayDomainImpl] {
	...
	if !cfg_node_exists {
		interpreter.output_handler.write_error(create_empty_node_location(), 0, fmt.Sprintf("The designated CFG Node %s doesn't exist", in_state.node_location))
	}
	...
	if is_cond_node && cond_node.Is_loop_head && interpreter.function_pre_mem_map[func_name].n_visits[in_state.node_location.Id] > interpreter.Settings.Loop_iters_before_widening {
		...
		if !state_changed && interpreter.function_pre_mem_map[func_name].n_visits[in_state.node_location.Id] > 0 {
			...
			// switch cfg_node_ty := cfg_node.(type) {
			// case *CFGNode:
			// 	interpreter.output_handler.write_info(in_state.node_location, cfg_node_ty.Line_num, "No updates to node state")
			// case *CFGCondNode:
			// 	interpreter.output_handler.write_info(in_state.node_location, cfg_node_ty.Line_num, "No updates to node state")
			// default:
			// 	panic("unreachable")
	...
	case *CFGNode:
		...
		case *imp.ScanfStmt:
			...
			if len(strings.Split(stmt.Format_string, " ")) != len(stmt.Assign_locations) {
				interpreter.output_handler.write_error(in_state.node_location, cfg_node.Line_num, "Scanf must have matching amount of format specifiers and assignment locations")
			}
			for index, fmt_str := range strings.Split(stmt.Format_string, " ") {
				...
				case "%t":
					...
					interpreter.set_abstract_value_from_expr(stmt.Assign_locations[index], default_val, &in_state)
				default:
					interpreter.output_handler.write_error(in_state.node_location, cfg_node.Line_num, fmt.Sprintf("Unknown scanf format specifier '%s'. only %%d for integers and %%t for bools allowed", fmt_str))
				}
			}
		case *imp.CallStmt:
	...
	case *CFGCondNode:
		...
		if !is_cond_edge {
			interpreter.output_handler.write_error(in_state.node_location, cfg_node.Line_num, "Condition stmt does not have outgoing edge of CondEdge type.")
		}
		...
		if !simpleprop_success {
			interpreter.output_handler.write_warning(in_state.node_location, cfg_node.Line_num, fmt.Sprintf("Could not represent '%s' as SimpleProp. Analysis precision may severely deterioriate.", cfg_node.Cond_expr))
		}
