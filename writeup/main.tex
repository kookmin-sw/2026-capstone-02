...
\begin{figure}[t]
    ...
    \caption{\imp의 명령문(statement), 수식(expression) 문법}
...
그림 \ref{fig:int_lattice}\footnote{Patrick Cousot: Principle of Abstract Interpretation의 33.1장에서 발췌함}는 정수 요약도메인을 격자로 표현한 그림이다.
...
Widening 연산은 일정 횟수 이상 상한(혹은 하한)이 단조 증가하는 경우 정확도를 희생하여 모든 정수들의 상계(혹은 하계)인 $\pm \infty$를 치환하여 분석이 종료할 수 있도록 유도하는 역할을 한다.
...
간략화된 AST가 생성된 이후 \traceinspector는 \verb|if, while|등의 명령문에 의해 프로그램의 실행 흐름을 구조화하기 위한 제어 흐름 그래프(control-flow graph, CFG)구조로 번역한다.
함수별 AST에서 CFG를 생성하는 알고리즘은 다음과 같이 동작한다:
\begin{enumerate}
    \item 함수의 진입 명령문으로부터 깊이 우선 탑색을 통해 함수의 종말 명령문부터 노드로 번역하여 ID를 부여한다.
...
\imp의 구체적인 실행의미론에선 프로그램 상태 $\sigma : \mathbb{V} \rightarrow \mathbb{v}$는 각 변수를 값으로 대응시켰다(부록 참조). 
...
따라서 초기 프로그램 상태에서부터 각 노드에 대해 요약실행 의미론을 적용하여 노드별 상태를 계속해서 갱신하고, 더 이상 변화가 생기지 않는 \textbf{고정점}에 도달할 때 까지 반복 수행한다. 
...
\begin{utheorem}[\traceinspector 분석기의 논리적 건전성]
    ...
    가 성립힌다.
\end{utheorem}
증명은 \imp의 명령문에 대한 구조적 귀납으로 실시한다. 
