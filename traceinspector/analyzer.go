...
func (interpreter *AbstractAnalyzer[IntDomainImpl, ArrayDomainImpl]) Eval_expr(abs_state AbstractState[IntDomainImpl, ArrayDomainImpl], expr imp.Expr) AbstractValue[IntDomainImpl, ArrayDomainImpl] {
	...
	case *imp.NegExpr:
		...
		if subexpr_val.domain_kind != IntDomainKind {
			interpreter.output_handler.write_error(abs_state.node_location, fmt.Sprintf("Result of arithmetic negation returned %s instead of IntDomain", subexpr_val.domain_kind))
		}
...
func (analyzer *AbstractAnalyzer[IntDomainImpl, ArrayDomainImpl]) Run_post_checks() {
	for func_name, cfg := range analyzer.Function_cfgs {
		for node_id, node := range cfg.Node_map {
			...
			if is_cond_node {
				node_eval_info, info_exists := analyzer.function_pre_mem_map[func_name].cond_node_condexpr_vals[node_id]
				if info_exists {
...
func Test(func_cfg_map FunctionCFGMap, func_name imp.ImpFunctionName, func_info_map imp.ImpFunctionMap, debug bool) {
	...
	for fun_name, val := range g.function_pre_mem_map {
		...
		for node_id, node_state := range val.pre_mem_node_map {
			...
			case *CFGNode:
				updated_nodes[node_id] = &CFGNode{Loc: node.Loc, Code: node_state.String() + "\n" + node.Code, Node_type: node.Node_type}
			case *CFGCondNode:
				updated_nodes[node_id] = &CFGCondNode{Loc: node.Loc, Code: node_state.String() + "\n" + node.Code, Node_type: node.Node_type}
			}
		}
